...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const { testId, questionId, recordingDuration, disease } = req.body;

      if (!testId || !questionId || !recordingDuration || !disease) {
        return res.status(400).json({ error: "Missing required fields" });
      }
      ...
      // GCS object path (IMPORTANT)
      const gcsFilePath = `raw/${disease}/${userId}/${testId}_${questionId}_${timestamp}.webm`;
      ...
      stream.on("error", (err) => {
        ...
        return res
          .status(500)
          .json({ error: "Failed to upload video to cloud" });
      });

      stream.on("finish", async () => {
        // Save DB record ONLY after successful upload
        const response = new Response({
          testId,
          questionId: parseInt(questionId),
          videoPath: `gs://${process.env.GCS_BUCKET_NAME}/${gcsFilePath}`,
...
router.get("/results/:testId", requireAuth, async (req, res) => {
  try {
    ...
    const totalScore = test.mcqAnswers.reduce(
